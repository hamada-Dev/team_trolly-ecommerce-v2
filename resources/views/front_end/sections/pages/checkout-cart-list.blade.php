                ...
                @foreach ($response->data->product_list as $item)
                    <div class="mini-cart-item">
                        <div class="mini-cart-image">
                            ...
                            <a href="{{ route('page.product',[$slug,$item->product_id]) }}" title="SPACE BAG">
                                <img src="{{get_file($item->image , APP_THEME())}}"alt="img">
                            </a>
                        </div>
                        <div class="mini-cart-details">
                            <p class="mini-cart-title">
                                <a href="{{ route('page.product',[$slug,$item->product_id]) }}">{{ $item->name }}</a>
                            </p>
        ...
        <div class="mini-cart-footer">
            ...
            <div class="mini-cart-footer-total-row d-flex align-items-center justify-content-between">
                ...
                <div class="mini-total-price subtotal">
                    {{  SetNumberFormat($response->data->final_price ?? ($response->data->sub_total ?? 0)) }}
                </div>
            </div>
            @if($plan->shipping_method == 'on')
                <div class="u-save d-flex justify-end">
            ...
            <div class="u-save d-flex justify-end">
                {{ __('Coupon') }} : <span class="discount_amount_currency"> - {{  SetNumberFormat($response->data->total_coupon_price ?? 0) }} </span>
            </div>
