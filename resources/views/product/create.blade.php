...
@endphp
@section('content')
    {{ Form::open(['route' => 'product.store', 'method' => 'post', 'id' => 'choice_form', 'enctype' => 'multipart/form-data', 'novalidate' => 'novalidate']) }}
    <div class="row pt-4">
        <div class="col-md-12">
            <div class="row">
                        ...
                        <div class="card-body">
                            @foreach (config('translation.languages') as $code => $language)
                                <div class="form-group">
                                    ...
                                    {{ Form::textarea("description[$code]", null, ['class' => 'form-control  smmernote-simple-product', 'rows' => 8, 'placeholder' => __('Product Description'), 'id' => "description[$code]"]) }}
                                </div>
                            @endforeach
                            @foreach (config('translation.languages') as $code => $language)
                                <div class="form-group">
                                    {{ Form::label('specification', __($language['label']) .' '. __('Product Specification'), ['class' => 'form-label']) }}
                                    {{ Form::textarea("specification[$code]", null, ['class' => 'form-control  ummernote-simple-product', 'rows' => 8, 'placeholder' => __('Product Specification'), 'id' => "specification[$code]"]) }}
                                </div>
                            @endforeach
                            @foreach (config('translation.languages') as $code => $language)
                                <div class="form-group">
                                    {{ Form::label('detail', __($language['label']) .' '. __('Product Detail'), ['class' => 'form-label']) }}
                                    {{ Form::textarea("detail[$code]", null, ['class' => 'form-control  ummernote-simple-product', 'rows' => 8, 'placeholder' => __('Product Detail'), 'id' => 'detail']) }}
                                </div>
                            @endforeach
                        </div>
                    </div>
