...
use App\Models\{Customer, Country, Order, PlanOrder, Plan, PlanCoupon, PlanRequest, Store, Setting, User,OrderBillingDetail , PixelFields};
use App\Models\Faq;
...
class HomeController extends Controller
{
    ...
    public function product_page(Request $request ,$storeSlug)
    {
        $store = Store::where('slug',$storeSlug)->first();
        $store_id = $store->id;
        $slug = $store->slug;
        ...
        $compact = ['slug','MainCategoryList','SubCategoryList','bestSeller','currency', 'currency_icon', 'min_price', 'max_price','product_count','has_subcategory','filter_tag','search_products','sub_category_select','featured_products','filter_product','MainCategory','homeproducts', 'products','product_list'];
    ...
    public function product_page_filter(Request $request, $storeSlug)
    {
        ...
        return view('front_end.sections.pages.product_list_filter', compact('tax_option','currentTheme','slug','products', 'currency', 'page','currency_icon','currentDateTime'))->render();
    }
    ...
    public function search_products(Request $request ,$slug)
    {
        ...
        if (!$products->isEmpty()) {
            ...
            foreach ($products as $product) {
                $url = route('page.product', [$slug, $product->slug]);
