...
class ProductController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('product.create' ,compact('link','MainCategory','ProductAttribute','Tax','Tax_status','Shipping','preview_type','tag'));
    ...
    public function edit(Product $product)
    {
        ...
        $compact =['link','product','MainCategory','Tax','Tax_status','Shipping','preview_type','ProductAttribute','SubCategory','product_image','get_tax','get_datas','tag' ,'get_tags'];
        ...
        if ($request->ajax() && isset($lastsegment) && !empty($lastsegment)) {
            ...
            if ($request->cat_id !== '' && $request->search == '') {
                if ($request->cat_id == '0') {
                    ...
                    $products = Product::where('maincategory_id', $request->cat_id)->where('variant_product', '0')->where('store_id', getCurrentStore())->where('theme_id', $store_id->theme_id)->get();
                }
            } else {
                if ($request->cat_id == '0') {
    ...
    public function addToCart(Request $request, $id, $session_key)
    {
        ...
        if ($product->track_stock == 0 && $product->stock_status == 'out_of_stock' ||
            ($product->track_stock != 0 && $product->product_stock < $settings['out_of_stock_threshold'] && $product->stock_order_status == 'not_allow') ||
            (!$product || ($session_key != 'pos' ))) {
        ...
        if (!$cart) {
            ...
            if ((($product->track_stock != 0 && $originalquantity < $cart[$id]['quantity']) || ($product->track_stock == 0 && $product->stock_status == 'out_of_stock')) && $session_key != 'pos')
            {
                return response()->json(
                    [
                        'code' => 404,
