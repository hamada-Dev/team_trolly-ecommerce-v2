...
class AccountProfileController extends Controller
{
    ...
    public function wish_list(Request $request, $slug)
    {
        // if (auth()->user()) {
            ...
            $wishlists = Wishlist::where('customer_id', auth('customers')->user()->id)->where('theme_id',  APP_THEME())->paginate(10);
            $wishlist_count = Wishlist::where('customer_id', auth('customers')->user()->id)->where('theme_id',  APP_THEME())->count();
